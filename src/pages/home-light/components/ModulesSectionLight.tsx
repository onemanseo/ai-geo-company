...
const modules = [
  {
    ...
    impact: "High",
    impactClass: "bg-teal-50 text-teal-600 border-teal-200",
  ...
  },
  {
    ...
    impact: "Medium-High",
    impactClass: "bg-amber-50 text-amber-600 border-amber-200",
    icon: "ri-route-line",
  },
  {
    name: "AI Retention",
    ...
    impact: "Medium",
    impactClass: "bg-orange-50 text-orange-500 border-orange-200",
...
export default function ModulesSectionLight() {
  ...
  return (
    <section id="modules-light" className="bg-white py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <p className="text-teal-500 text-xs font-semibold tracking-widest uppercase mb-4">The Modules</p>
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">8 AI Modules</h2>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto leading-relaxed">
            Each module solves a specific growth problem. Together they form a compounding system.{" "}
            <span className="text-gray-800 font-medium">Click any card to see the solution.</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Row 1 */}
          <div ref={row1.ref} className="contents">
            {modules.slice(0, 4).map((mod, i) => (
              <div
                key={mod.name}
                className={`relative rounded-2xl border bg-white p-5 cursor-pointer flex flex-col transition-all duration-500 ease-out ${
                  expanded === i
              ...
              >
                <span
                  className={`absolute top-4 right-4 px-2 py-0.5 rounded-full text-[10px] font-bold border ${mod.impactClass} whitespace-nowrap`}
                >
                  {mod.impact}
                </span>
                <div className="mb-4">
                  <div className="w-11 h-11 flex items-center justify-center rounded-xl bg-teal-50 border border-teal-200">
                    <i className={`${mod.icon} text-teal-500 text-xl`}></i>
                  </div>
                </div>
                <p className="text-gray-400 text-[10px] font-semibold tracking-widest uppercase mb-1">{mod.subtitle}</p>
                <h3 className="text-gray-900 text-base font-bold mb-3 leading-snug">{mod.name}</h3>
                <p className="text-gray-500 text-sm leading-relaxed flex-1">
                  <span className="text-red-400 font-medium">Pain: </span>
                  {mod.pain}
                </p>
                <div
                ...
                >
                  <div className="pt-3 border-t border-gray-200">
                    <p className="text-sm leading-relaxed">
                      <span className="text-teal-600 font-medium">Solution: </span>
                      <span className="text-gray-700">{mod.solution}</span>
                    </p>
                  </div>
                </div>
          ...
          {/* Row 2 */}
          <div ref={row2.ref} className="contents">
