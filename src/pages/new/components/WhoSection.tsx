...
export default function WhoSection() {
  ...
  const segments = [
    ...
    },
    {
      ...
      img: "https://readdy.ai/api/search-image?query=marketplace%20aggregator%20platform%20multiple%20product%20categories%20diverse%20listings%20comparison%20shopping%20website%20clean%20modern%20UI%20desktop%20interface%20professional%20tech%20platform&width=480&height=280&seq=who-02&orientation=landscape",
    },
    {
      ...
      img: "https://readdy.ai/api/search-image?query=luxury%20premium%20product%20high%20consideration%20purchase%20automotive%20real%20estate%20B2B%20professional%20service%20elegant%20minimal%20website%20design%20sophisticated%20brand%20identity&width=480&height=280&seq=who-03&orientation=landscape",
    },
  ];

  return (
    <section id="who" className="bg-[#0A0A0B] py-16 lg:py-20">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <p className="text-[#00E5CC] text-xs font-semibold tracking-widest uppercase mb-4">Ideal Fit</p>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Who It&apos;s For</h2>
        ...
        <div ref={cards.ref} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {segments.map((seg, i) => (
            <div
              key={seg.label}
              className={`rounded-2xl border border-white/8 bg-[#111113] overflow-hidden hover:border-[#00E5CC]/30 transition-all duration-500 ease-out group ${
              ...
              <div className="relative h-44 overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#0A0A0B]/80 backdrop-blur-sm border border-white/10">
                  <div className="w-4 h-4 flex items-center justify-center">
                    <i className={`${seg.icon} text-[#00E5CC] text-sm`}></i>
                  </div>
                  <span className="text-white text-xs font-semibold">{seg.label}</span>
                </div>
              </div>

              <div className="p-5">
                <h3 className="text-white text-lg font-bold mb-2 leading-snug">{seg.title}</h3>
                <p className="text-[#8A8A8A] text-sm leading-relaxed mb-4">{seg.desc}</p>

                {/* Wins */}
                <div className="mb-4">
                  <p className="text-[#8A8A8A] text-xs font-semibold tracking-widest uppercase mb-2">Key Modules</p>
                  <div className="flex flex-wrap gap-2">
                    {seg.wins.map((w) => (
                      <span
                        key={w}
                        className="px-2.5 py-1 rounded-full bg-[#00E5CC]/10 border border-[#00E5CC]/20 text-[#00E5CC] text-xs font-medium whitespace-nowrap"
                      >
                        {w}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {seg.metrics.map((m) => (
                    <div key={m.label} className="rounded-xl bg-[#1A1A1C] p-3">
                      <p className="text-white text-sm font-bold mb-0.5">{m.value}</p>
                      <p className="text-[#8A8A8A] text-xs">{m.label}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div
          ref={callout.ref}
          className={`rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center gap-4 transition-all duration-700 ease-out ${
            callout.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
          }`}
        >
          <div className="w-10 h-10 flex items-center justify-center rounded-full bg-amber-500/15 flex-shrink-0">
